...
func setRouter(s *web.EasyServer) {
	...
	s.AddHandler("POST", "/api/nodes/stop", func(ctx web.Context) {
		ctx.Writer.Write(StopNodes())
	})

	s.AddHandler("POST", "/api/nodes/start-selected", func(ctx web.Context) {
		ctx.Writer.Write(StartSelectedNodes())
	})

	s.AddHandler("POST", "/api/nodes/stop-selected", func(ctx web.Context) {
