...
func (p *ProxyPool) InitSubscribeData() *ProxyPool {
	...
	if dt != "" {
		...
		validIndex := 0
		for _, vnd := range vnds {
			// 过滤不支持的节点
			if !p.isNodeSupported(vnd) {
				continue
			}
			pnd := p.getNodeByV2rayNode(vnd, validIndex)
...
// isNodeSupported 检查节点是否使用支持的协议
func (p ProxyPool) isNodeSupported(vnd V2rayNode) bool {
	// 过滤不支持的传输协议
	unsupportedTransports := []string{"httpupgrade", "grpc"}
	for _, unsupported := range unsupportedTransports {
		if strings.ToLower(vnd.Net) == unsupported {
			logger := getConf().GetLogger()
			logger.Warnf("跳过不支持的传输协议节点: %s (协议: %s)", vnd.Ps, vnd.Net)
			return false
		}
	}
	return true
...
func SetSystemProxy(proxyAddr string, enable bool) error {
	if enable && proxyAddr != "" {
		return SetProxy(proxyAddr)
	} else {
		return SetProxy("")
	}
}
...
func GetSystemProxyStatus() (bool, string) {
	// 这里简化实现，实际应该读取系统代理设置
	// 由于Windows API比较复杂，这里返回基本状态
	return false, ""
}
...
func (p *ProxyPool) SetLocalAddr(n *ProxyNode, port int) string {
	if port == 0 {
		// 自动查找可用端口，避免端口冲突
		expectedPort := n.Index + p.localPortStart
		n.LocalPort = netutil.FindAvailablePort(expectedPort)
		if n.LocalPort != expectedPort {
			logger := getConf().GetLogger()
			logger.Warnf("端口 %d 被占用，自动分配端口 %d 给节点 %d", expectedPort, n.LocalPort, n.Index)
		}
	} else {
		n.LocalPort = port
	}
...
func (p *ProxyPool) StartAll() error {
	...
	logger := getConf().GetLogger()
	logger.Infof("------StartAll--完成--成功启动节点数: %d--", successCount)
